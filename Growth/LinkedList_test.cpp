...
void LinkedList::middle_pop(int idx){
    if(empty()){
        cout << "empty" << '\n';
        return;
    }

    if(idx < 0 || idx >= N) { cout << "Wrong Number" << '\n'; return; }
...
int main() {
    ...
    while(n--){
        ...
        else if(cmd == "middlePop"){
            int temp;
            cout << "What index do you want to pop : ";
            cin >> temp;

            link.middle_pop(temp);
        }
        else if(cmd == "empty"){
